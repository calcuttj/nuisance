...
Smearceptance_Tester::Smearceptance_Tester(nuiskey samplekey) {
  ...
  ERecDistrib = NULL;
  ERecDistrib_CC0Pi = NULL;
  ERecDistrib_CC1Pi = NULL;
  ERecDistrib_CCOther = NULL;
  RecoSmear = NULL;
  ...
  if (RecBinL != 0xdeadbeef) {
    ...
    ERecDistrib_CC1Pi = new TH1D("ELepRec_rate_CC1Pi", ";Rec E_{#nu};Count", RecNBins,
                           RecBinL, RecBinH);
                           
    ERecDistrib_CCOther = new TH1D("ELepRec_rate_CCOther", ";Rec E_{#nu};Count", RecNBins,
                           RecBinL, RecBinH);
    ...
    RecoSmear_CC0Pi =
        new TH2D("ELepHadVis_Recon_CC0Pi", ";True E_{#nu};Recon. E_{#nu}", RecNBins,
                 RecBinL, RecBinH, TrueNBins, TrueBinL, TrueBinH);
    RecoSmear_CC0Pi->Sumw2();

    RecoSmear_CC1Pi =
        new TH2D("ELepHadVis_Recon_CC1Pi", ";True E_{#nu};Recon. E_{#nu}", RecNBins,
                 RecBinL, RecBinH, TrueNBins, TrueBinL, TrueBinH);
    RecoSmear_CC1Pi->Sumw2();

    RecoSmear_CCOther =
        new TH2D("ELepHadVis_Recon_CCOther", ";True E_{#nu};Recon. E_{#nu}", RecNBins,
                 RecBinL, RecBinH, TrueNBins, TrueBinL, TrueBinH);
    RecoSmear_CCOther->Sumw2();
  }
...
void Smearceptance_Tester::Write(std::string drawOpt) {
  ...
  TH2D *SmearMatrix_ev_CC0Pi =
      static_cast<TH2D *>(RecoSmear_CC0Pi->Clone("ELepHadVis_Smear_ev_CC0Pi"));

  for (Int_t trueAxis_it = 1;
       trueAxis_it < RecoSmear_CC0Pi->GetXaxis()->GetNbins() + 1; ++trueAxis_it) {
    double NEISLep = ETrueDistrib_noweight->GetBinContent(trueAxis_it);

    for (Int_t recoAxis_it = 1;
         recoAxis_it < RecoSmear_CC0Pi->GetYaxis()->GetNbins() + 1; ++recoAxis_it) {
      if (NEISLep > std::numeric_limits<double>::epsilon()) {
        SmearMatrix_ev_CC0Pi->SetBinContent(
            trueAxis_it, recoAxis_it,
            SmearMatrix_ev_CC0Pi->GetBinContent(trueAxis_it, recoAxis_it) / NEISLep);
      }
    }
  }

  TH2D *SmearMatrix_ev_CC1Pi =
      static_cast<TH2D *>(RecoSmear_CC1Pi->Clone("ELepHadVis_Smear_ev_CC1Pi"));

  for (Int_t trueAxis_it = 1;
       trueAxis_it < RecoSmear_CC1Pi->GetXaxis()->GetNbins() + 1; ++trueAxis_it) {
    double NEISLep = ETrueDistrib_noweight->GetBinContent(trueAxis_it);

    for (Int_t recoAxis_it = 1;
         recoAxis_it < RecoSmear_CC1Pi->GetYaxis()->GetNbins() + 1; ++recoAxis_it) {
      if (NEISLep > std::numeric_limits<double>::epsilon()) {
        SmearMatrix_ev_CC1Pi->SetBinContent(
            trueAxis_it, recoAxis_it,
            SmearMatrix_ev_CC1Pi->GetBinContent(trueAxis_it, recoAxis_it) / NEISLep);
      }
    }
  }

  TH2D *SmearMatrix_ev_CCOther =
      static_cast<TH2D *>(RecoSmear_CCOther->Clone("ELepHadVis_Smear_ev_CCOther"));

  for (Int_t trueAxis_it = 1;
       trueAxis_it < RecoSmear_CCOther->GetXaxis()->GetNbins() + 1; ++trueAxis_it) {
    double NEISLep = ETrueDistrib_noweight->GetBinContent(trueAxis_it);

    for (Int_t recoAxis_it = 1;
         recoAxis_it < RecoSmear_CCOther->GetYaxis()->GetNbins() + 1; ++recoAxis_it) {
      if (NEISLep > std::numeric_limits<double>::epsilon()) {
        SmearMatrix_ev_CCOther->SetBinContent(
            trueAxis_it, recoAxis_it,
            SmearMatrix_ev_CCOther->GetBinContent(trueAxis_it, recoAxis_it) / NEISLep);
      }
    }
  }
  ...
  RecoSmear_CC1Pi->Write();
  RecoSmear_CCOther->Write();

  SmearMatrix_ev->Write();
  SmearMatrix_ev_CC0Pi->Write();
  SmearMatrix_ev_CC1Pi->Write();
  SmearMatrix_ev_CCOther->Write();
